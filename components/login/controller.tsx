...
type Props = {
    ...
    experimentalPrimaryTeam: string | null;
...
class LoginController extends React.PureComponent<Props, State> {
    private loginIdInput: React.RefObject<HTMLInputElement>;
    private passwordInput: React.RefObject<HTMLInputElement>;
    private closeSessionExpiredNotification: (() => void) | null;
    ...
    constructor(props: Props) {
        ...
        if (this.props.location?.search) {
            ...
            if (searchParams.get('extra') === Constants.SIGNIN_VERIFIED && searchParams.get('email')) {
                loginId = searchParams.get('email') || '';
            }
        }
    ...
    createLoginOptions = () => {
        ...
        if (gitlabSigninEnabled) {
            ...
            if (this.props.gitlabButtonText) {
                buttonText = this.props.gitlabButtonText as unknown as JSX.Element;
            }
            loginControls.push(
                <a
                    id='GitLabButton'
        ...
        if (openIdSigninEnabled) {
            ...
            if (this.props.openidButtonText) {
                buttonText = this.props.openidButtonText as unknown as JSX.Element;
            }
            loginControls.push(
                <a
                    id='OpenIdButton'
